...
/**
 * Creates one mtext entity using the inline editor.
 */
export async function createMTextEntity(
  ...
  mtextEditor = new AcEdMTextEditor()
    ...
    const textHeight = pixelsToWorldY(view, 24)
    ...
    const result = await mtextEditor.open({
      view,
      location,
      width,
      textHeight,
...
export class AcApMTextCmd extends AcEdCommand {
  ...
  async execute(context: AcApContext) {
    await createMTextEntity(context, this.mtextEditor)
  }
}
