...
export function injectParsingDetailsStyles() {
  ...
  style.id = STYLE_ID
  style.textContent = `
    .parsing-details-modal { position: fixed; z-index: 970; inset: 0; display: grid; place-items: center; padding: 20px; color: var(--app-text, #17262b); background: var(--app-overlay-scrim, rgba(12, 28, 34, .64)); }
    .parsing-details-modal[hidden] { display: none; }
    .parsing-details-shell { width: min(560px, 100%); max-height: min(680px, 100%); overflow: auto; border: 1px solid var(--app-border-strong, #b9c7ca); border-radius: var(--app-radius-panel, 6px); background: var(--app-surface-panel, #f8fafa); box-shadow: var(--app-shadow-modal, 0 20px 54px rgba(3, 18, 23, .38)); }
    ...
    .parsing-details-shell > header > span { display: grid; place-items: center; width: 34px; height: 34px; color: var(--app-accent, #087b58); background: var(--app-success-surface, #e3f5ee); }
    ...
    .parsing-details-shell header button { display: grid; place-items: center; width: 34px; min-height: 34px; border: 1px solid var(--app-border-strong, #bcc9cc); border-radius: var(--app-radius-control, 4px); padding: 0; color: inherit; background: var(--app-surface-elevated, #fff); cursor: pointer; }
