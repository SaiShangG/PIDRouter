...
export const getDemoToolbarLayouts = (
  locale: AppLocale
): AcExToolbarLayoutPreset[] =>
  [
  ...
  ].map(layout => ({ ...layout, label: translateUiText(locale, layout.label) }))

/** Full toolbar `items` for each demo layout preset. */
const getDemoToolbarPresetItems = (
  locale: AppLocale
): Record<string, AcExToolbarItemsInput> => {
  const t = (text: string) => translateUiText(locale, text)
  return {
    default: 'default',
    minimal: [
    ...
    custom: [
      ...
      },
      {
        ...
        requiresDocument: true,
        children: [
          { id: 'demo-zoom-all', label: t('Zoom All'), command: 'zoom\nall' },
          { id: 'demo-zoom-window', label: t('Zoom Window'), command: 'zoom\nw' }
        ]
      },
      {
        ...
        toggle: {
          ...
          on: {
            label: t('Hide Demo Tools'),
            action: () => {
              demoCustomToolsVisible = false
            }
          },
          off: {
            label: t('Show Demo Tools'),
            action: () => {
      ...
      },
      {
        ...
        action: () => {
          ...
          window.alert(t(`Layer count: ${count}`))
        }
      },
      toolbarPreset('layer')
    ]
...
export function applyDemoToolbarLayout(
  ...
  const items = getDemoToolbarPresetItems(locale)[presetId]
  if (!items) return
  ...
  const layoutSwitcher = createToolbarLayoutSwitcher({
    presets: getDemoToolbarLayouts(locale),
    currentId: presetId,
    onSelect: id => applyDemoToolbarLayout(plugin, id, locale)
  })
