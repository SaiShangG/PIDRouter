...
export function injectConfirmationModalStyles() {
  ...
  style.id = STYLE_ID
  style.textContent = `
    .confirmation-modal { position: fixed; z-index: 980; inset: 0; display: grid; place-items: center; padding: 20px; color: var(--app-text, #17262b); background: var(--app-overlay-scrim, rgba(12, 28, 34, .62)); }
    .confirmation-modal[hidden] { display: none; }
    .confirmation-modal-shell { width: min(430px, 100%); overflow: hidden; border: 1px solid var(--app-border-strong, #b9c7ca); border-radius: var(--app-radius-panel, 6px); background: var(--app-surface-panel, #f8fafa); box-shadow: var(--app-shadow-modal, 0 20px 54px rgba(3, 18, 23, .38)); }
    ...
    .confirmation-modal-shell footer { display: flex; justify-content: flex-end; gap: 8px; padding: 12px 16px; border-top: 1px solid var(--app-border, #d2dcde); background: var(--app-surface, #edf2f2); }
    .confirmation-modal button { min-height: 34px; border: 1px solid var(--app-border-strong, #b9c7ca); border-radius: var(--app-radius-control, 4px); padding: 6px 14px; color: var(--app-text, #17262b); background: var(--app-surface-elevated, #fff); font: inherit; cursor: pointer; }
