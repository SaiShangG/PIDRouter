...
const pairs: TranslationPair[] = [
  ...
  ['按序列筛选', 'Filter by sequence'], ['全部序列', 'All sequences'],
  ['批量排除', 'Exclude filtered'], ['批量恢复', 'Restore filtered'],
  ...
  ['删除生成记录', 'Delete generated file'], ['关闭 PDF 预览', 'Close PDF preview'],
  ['上一页', 'Previous page'], ['下一页', 'Next page'], ['PDF 页码', 'PDF page number'],
  ['缩小', 'Zoom out'], ['放大', 'Zoom in'], ['适合宽度', 'Fit to width'],
  ['PDF 页面预览', 'PDF page preview'], ['正在加载 PDF 预览…', 'Loading PDF preview…'],
  ['PDF 预览加载失败', 'Failed to load PDF preview'],
  ['文件名', 'File name'], ['输入 PDF 文件名', 'Enter PDF filename'],
  ['自定义 PDF 文件名', 'Custom PDF filename'], ['导出范围', 'Export scope'],
  ['当前序列', 'Current sequence'], ['选中序列', 'Selected sequences'],
  ...
  ['正在取消…', 'Canceling…'],
  ['导出设备 Matrix', 'Export device matrix'], ['文件格式', 'File format'],
  ['Matrix 文件格式', 'Matrix file format'],
  ['Operation / Phase 范围', 'Operation / Phase scope'],
  ['设备与内容', 'Devices and content'],
  ['包含未激活设备', 'Include inactive devices'],
  ['包含转换条件', 'Include transition conditions'],
  ['导出 Matrix', 'Export matrix'],
  ['Matrix 正在后台生成，请稍候', 'Generating matrix on the server. Please wait.'],
  ['Matrix 导出完成', 'Matrix export completed'],
  ['Matrix 导出已取消', 'Matrix export canceled'],
  ['Matrix 导出失败', 'Matrix export failed'],
...
export const translateUiText = (locale: AppLocale, text: string): string => {
  ...
  const dynamic: Array<[RegExp, (match: RegExpMatchArray) => string]> = locale === 'en'
    ? [
      ...
      [/^已选择 (\d+) 个 Operation，(\d+) 个 Phase$/, m => `${m[1]} Operations and ${m[2]} Phases selected`],
