...
import { AcTrMTextColorUtil, AcTrSubEntityTraitsUtil } from '../util'
...
export class AcTrRenderer implements AcGiRenderer<AcTrEntity> {
  ...
  /**
   * Builds final MTEXT glyph geometry through this renderer's initialized text pipeline.
   * Used by vector exporters that need the exact viewer glyph outlines.
   */
  renderMTextGeometry(
    mtext: AcGiMTextData,
    style: AcGiTextStyle,
    ...
    return AcTrMTextRenderer.getInstance().syncRenderMText(
      mtext as MTextData,
      style as TextStyle,
      AcTrMTextColorUtil.buildColorSettingsFromTraits(
        traits,
        backgroundColor
      )
    )
