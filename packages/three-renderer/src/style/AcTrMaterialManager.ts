...
export abstract class AcTrMaterialManager<T> {
  ...
  changeWhiteDisplayForBackground(backgroundColor: number) {
    for (const oldKey of Object.keys(this.cache)) {
      const oldMaterial = this.cache[oldKey]
      const metadata = getMaterialMetadata(oldMaterial)
      if (metadata.isForeground === true || metadata.isBackgroundFill === true) {
        continue
      }
      if (metadata.sourceColorRgb !== WHITE_RGB) continue

      AcTrMaterialUtil.setMaterialColor(
        oldMaterial,
        new THREE.Color(
          this.resolveDisplayRgbForBackground(WHITE_RGB, backgroundColor)
        )
      )
    }
  }
  ...
  protected createMaterial(
    ...
    const sourceColorRgb = this.resolveSourceMaterialRgb(traits, layerColorRgb)
  ...
  private resolveDisplayRgbForBackground(
    rgb: number,
    backgroundColor = this.options.currentBackgroundColor
  ): number {
    return rgb === WHITE_RGB && acgiIsLightBackground(backgroundColor)
      ? BLACK_RGB
      : rgb
  }
