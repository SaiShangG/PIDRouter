...
describe('buildSvgMText', () => {
  ...
  it('anchors width-factor scaling at the text span origin', () => {
    const { localSvg } = buildSvgMText(
      {
        text: '3/4\\C1;"OD',
        height: 10,
    ...
    expect(localSvg).toContain(
      'transform="translate(0,0) scale(0.425,1) translate(0,0)"'
    )
    expect(localSvg).toMatch(
      /transform="translate\((?!0(?:\.0+)?,0\))([\d.]+),0\) scale\(0\.425,1\) translate\(-\1,0\)"/
    )
    expect(localSvg).not.toContain('transform="scale(0.425,1)"')
  })
  ...
  it('does not wrap PID labels for a degenerate reference width', () => {
    const { localSvg } = buildSvgMText(
      {
        text: '1/2"OD-D4',
        height: 10,
    ...
    const tspans = [...localSvg.matchAll(/<tspan/g)]
    expect(tspans).toHaveLength(1)
    expect(localSvg).toContain('1/2&quot;OD-D4')
  })
  ...
  it('uses explicit rotation instead of a conflicting direction vector', () => {
    const { localSvg } = buildSvgMText(
      {
        text: '1/2"OD-D4',
        height: 10,
  ...
  it('preserves an explicit zero rotation', () => {
    const { localSvg } = buildSvgMText(
      {
        text: 'Horizontal',
        height: 10,
    ...
    expect(localSvg).not.toContain('rotate(')
  })
