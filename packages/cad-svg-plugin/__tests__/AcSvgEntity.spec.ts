...
describe('AcSvgEntity transforms', () => {
  ...
  it('recolours only matching entity strokes without adding geometry', () => {
    const renderer = new AcSvgRenderer()
    const highlighted = renderer.lines([
      { x: 0, y: 0, z: 0 },
      { x: 10, y: 0, z: 0 }
    ])
    highlighted.objectId = 'flow-line'
    const unchanged = renderer.lines([
      { x: 0, y: 1, z: 0 },
      { x: 10, y: 1, z: 0 }
    ])
    unchanged.objectId = 'other-line'

    renderer.overrideEntityStrokeColors(new Set(['flow-line']), 0x00c853)

    const exported = renderer.export()
    expect(exported.match(/stroke="#00c853"/g)).toHaveLength(1)
    expect(exported.match(/<path /g)).toHaveLength(2)
  })

  it('preserves text fills when an entity stroke is recoloured', () => {
    const entity = new AcSvgEntity()
    entity.svg =
      '<path d="M0,0L1,0" stroke="#000000" fill="none"/>' +
      '<path d="M0,0L1,0L0,1Z" fill="#ff00ff" stroke="none"/>'

    entity.setStrokeColorOverride('#00c853')

    expect(entity.renderSvg()).toContain('stroke="#00c853" fill="none"')
    expect(entity.renderSvg()).toContain('fill="#ff00ff" stroke="none"')
  })

  it('recolours child linework when a grouped entity ID matches', () => {
    const renderer = new AcSvgRenderer()
    ...
    group.objectId = 'grouped-flow-line'

    renderer.overrideEntityStrokeColors(
      new Set(['grouped-flow-line']),
      0x00c853
    )
