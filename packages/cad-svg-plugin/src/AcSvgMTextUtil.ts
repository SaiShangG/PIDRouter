...
function resolveRotation(mtext: AcGiMTextData): number {
  if (typeof mtext.rotation === 'number' && Number.isFinite(mtext.rotation)) {
...
export function buildSvgMText(
  ...
  const wrapWidth = resolveWrapWidth(mtext, baseHeight)
...
class MTextSvgLayout {
  ...
  get tspanMarkup(): string {
    ...
    for (const line of this.lines) {
      ...
      for (const span of line.spans) {
        const attrs = this.tspanAttributes(
          span.tokenCtx,
          span.fontSize,
          span.x
        )
  ...
  private tspanAttributes(
    ...
    if (widthFactor !== 1 || oblique !== 0) {
      transforms.push(`translate(${originX},0)`)
    }
    if (widthFactor !== 1) {
      transforms.push(`scale(${widthFactor},1)`)
    }
    if (oblique !== 0) {
      transforms.push(`skewX(${-oblique})`)
    }
